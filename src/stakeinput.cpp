...
bool CBIDStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CBIDStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CBIDStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CBIDStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CBIDStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CBIDStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CBIDStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CBIDStake::GetUniqueness() const
{
...
CBlockIndex* CBIDStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CBIDStake::ContextCheck(int nHeight, uint32_t nTime)
{
