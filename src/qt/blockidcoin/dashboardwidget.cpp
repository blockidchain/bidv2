    ...
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareBID, "square-chart-blockidcoin");
    setCssProperty(ui->labelBID, "text-chart-blockidcoin");
    ...
    setCssProperty(ui->labelAmountBID, "text-stake-blockidcoin-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &blockidcoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &blockidcoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &blockidcoinGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            blockidcoin = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalBID += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesBID.append(blockidcoin);

        int max = std::max(blockidcoin, blockidcoin);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesBID);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalBID > 0) {
        setCssProperty(ui->labelAmountBID, "text-stake-blockidcoin");
    } else {
        setCssProperty(ui->labelAmountBID, "text-stake-blockidcoin-disable");
    }
    forceUpdateStyle({ui->labelAmountBID});
    ui->labelAmountBID->setText(GUIUtil::formatBalance(chartData->totalBID, nDisplayUnit));
