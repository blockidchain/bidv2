...
/** Fees smaller than this (in ublockidcoin) are considered zero fee (for relaying, mining and transaction creation)
...
int64_t GetDevPayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight <= 1000) { 
        ...
        nDevPayment = 350 * COIN;
    } else if (nHeight <= 490000) {
        nDevPayment = 500 * COIN;
    } else if (nHeight <= 500000) {
        nDevPayment = 450 * COIN;
    } else if (nHeight <= 510000) {
...
{
    util::ThreadRename("blockidcoin-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> blockidcoinInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                blockidcoinInputs.push_back(stakeIn);
        }
        const bool hasBIDInputs = !blockidcoinInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasBIDInputs) {
                    ...
                    for (const CTxIn& blockidcoinIn : blockidcoinInputs)
                        if(blockidcoinIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasBIDInputs) {
                            for (const CTxIn& stakeIn : blockidcoinInputs)
