...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("Blockidcoin CoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("Blockidcoin CoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("Blockidcoin CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("blockidcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running Blockidcoin CoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("Blockidcoin CoinMiner exception");
    } catch (...) {
        LogPrintf("Blockidcoin CoinMiner exception");
    }

    LogPrintf("Blockidcoin CoinMiner exiting\n");
}
